...
from classcomm.coordinator.forms import *
...
def index(request):
    ...
    CourseInfo=dict()
    if 'q' in request.GET :
        if request.GET['items']=='teacher':
            q = request.GET['q']
            UserInfo = User.objects.all().filter(username__contains=q).order_by('username')
            if not q:
                error = True
            else:
                pass
        if request.GET['items']=='course':
            q = request.GET['q']
            CourseInfo = Course.objects.all().filter(name__contains=q).order_by('name')
            if not q:
                error = True
            else:
                pass
    ...
    template = loader.get_template('coordinator/index.html')
    if error == True:
        context = RequestContext(request, {"error": error})
    else:
        if UserInfo:
            context = RequestContext(request, {"error": error,"UserInfo":UserInfo})
        else:
            context = RequestContext(request, {"error": error,"CourseInfo":CourseInfo})
...
def calendar(request):
    ...
    user = request.user
    global global_query_user
    global global_query_item
    if 'item' in request.GET:
        global_query_item = request.GET['item']
    if 'user' in request.GET:
        cal_user = request.GET['user']
        global_query_user = cal_user
    if global_query_item == 'user':
        user_access = User.objects.all().filter(username=global_query_user).values()
        uid = user_access[0]['id']
        courses = Course.objects.all().filter(Q(director=uid))
    if global_query_item =='course':
        courses = Course.objects.all().filter(Q(name=global_query_user))
    ...
    template = loader.get_template('coordinator/calendar.html')
...
def course_index(request, course_id):
    ...
    template = loader.get_template('coordinator/course_blocked.html')
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR, COURSE_MENTOR_LIMIT]:
        ...
        template = loader.get_template('coordinator/course_index.html')
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect(reverse(
                    'coordinator.views.course_index', args=[course.id,]))

        # Finally Set template, create Context and return Response
        template = loader.get_template('coordinator/course_announcement_add.html')
...
def course_announcement_delete(request, course_id, announcement_id):
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR]:
        if request.POST:
            try: # Grab Announcement from URL parameters
                ...
                return HttpResponseRedirect(reverse(
                    'coordinator.views.course_index', args=[course_id,]))
    ...
    return HttpResponseRedirect(reverse('coordinator.views.course_index', args=[course_id,]))
# End Def
...
def course_assignments(request, course_id):
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR, COURSE_MENTOR_LIMIT]:
        ...
        template = loader.get_template('coordinator/course_assignments.html')
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR]:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                LogEntry.objects.log_action(request.user.pk, ContentType.objects.get_for_model(instance).pk,
                                            instance.pk, force_unicode(instance), action_flag=ADDITION)
                return HttpResponseRedirect(reverse('coordinator.views.course_assignments', args=(course.id,)))

        # Set template, create Context and Return page view
        template = loader.get_template('coordinator/course_assignment_add.html')
        context = RequestContext(request, {"course": course, "form": form})
    return HttpResponse( template.render(context) )
# End View
...
def course_information(request, course_id):
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR, COURSE_MENTOR_LIMIT]:
        ...
        template = loader.get_template('coordinator/course_information.html')
...
def course_resources(request, course_id):
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR, COURSE_MENTOR_LIMIT]:
        ...
        # Set template; Create context
        template = loader.get_template('coordinator/course_resources.html')
        context = RequestContext(request, {"access_type": access_type, "course": course,
...
def checkscript_all(request):
    ...
    if request.user.is_superuser: # Get all checkscript submissions for super users
        ...
        template = loader.get_template('coordinator/checkscript.html')
        ...
        return HttpResponseRedirect(reverse('coordinator.views.checkscript', args=[]))
# EndView
...
def roster_tools(request, course_id):
    ...
    if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR,
        ...
        # Set template; Create Context
        template = loader.get_template('coordinator/course_roster.html')
...
def grade_report(request, course_id, enrollment_id):
    ...
    if enrollment.course.id == course.id: # Verify URL Correctness
        ...
        if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR, COURSE_MENTOR_LIMIT]:
            ...
            # Set template; Create Context
            template = loader.get_template('coordinator/grade_report.html')
...
def return_grade(request, enrollment_id, assignment_id):
        ...
        if access_type in [COURSE_DIRECTOR, COURSE_INSTRUCTOR, COURSE_MENTOR, COURSE_MENTOR_LIMIT]:
            ...
            if request.POST: # When POST: Create Grade and save obj when form passes validation
                ...
                if form.is_valid():
                    ...
                    LogEntry.objects.log_action(request.user.pk, ContentType.objects.get_for_model(instance).pk,
                                                instance.pk, force_unicode(instance), action_flag=ADDITION)
                    return HttpResponseRedirect(reverse('coordinator.views.grade_report',
    ...
    template = loader.get_template('coordinator/student_grade_return.html')
...
def delete_grade(request, course_id, enrollment_id, grade_id):
    ...
    return HttpResponseRedirect(reverse('coordinator.views.grade_report', args=[course_id, enrollment_id,]))
# End Def
...
def add_DDO(request, enrollment_id, assignment_id):
    ...
    # Set template, and create Context and Return page view
    template = loader.get_template('coordinator/student_DDO_add.html')
    context = RequestContext(request, {"form": form,
        "enrollment": enrollment,
        "course": enrollment.course,
        "assignment": assignment})
